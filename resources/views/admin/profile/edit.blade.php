    ...
    <div class="col-md-12">
      <form action="{{ route('admin.handleEdit',['id'=>$info['id']]) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <br>
          <img src="{{ URL::to('/uploads/images') }}/{{ $info['avatar'] }}" alt="avatar" width="120" height="120">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <select class="form-control" name="single" id="single">
            <option value="1" {{ $info['single'] == 1 ? 'selected=selected' : '' }}> Doc than </option>
            <option value="0" {{ $info['single'] == 0 ? 'selected=selected' : '' }}> Co gia dinh </option>
          </select>
        </div>
